...
export function StatsCard({ title, value, icon: Icon, color = "primary" }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted-foreground" data-testid="stats-title">
        {title}
      </p>
    </div>
  );
    ...
    >
      ...
      <div className="space-y-2 mb-4 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium text-primary">
            {new Date(request.date_time).toLocaleDateString("ar-SA")}
          </span>
        </div>
      </div>
...
export function AssignmentCard({ assignment, onViewDetails }) {
  return (
    ...
    >
        ...
        <div>
          ...
          {assignment.teacher_name || assignment.student_name ? (
            <p className="text-sm text-muted-foreground">
              {assignment.teacher_name && `المعلم: ${assignment.teacher_name}`}
              {assignment.teacher_name && assignment.student_name && " · "}
              {assignment.student_name && `الطالب: ${assignment.student_name}`}
            </p>
          ) : (
            <p className="text-sm text-muted-foreground">
      ...
      {assignment.jitsi_room &&
        assignment.status !== "completed" &&
        assignment.status !== "cancelled" && (
          <div className="mb-3 flex items-center gap-1 text-xs text-green-600 bg-green-50 px-2 py-1 rounded-md w-fit">
            <span>📹</span>
            <span>مكالمة فيديو متاحة</span>
          </div>
        )}
      ...
      {assignment.notes && (
        <p className="text-sm text-muted-foreground mb-4">{assignment.notes}</p>
      )}
